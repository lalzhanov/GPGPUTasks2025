                     ...
                     unsigned int height)
{
    ...
    // Код в обоих кернелах один и тот же, но размер рабочей группы в этом случае - 256 x 1, что представляет собой горизонтальную полоску - лучший с точки зрения memory coalesced паттерна случай.
    const unsigned int column = get_global_id(1);
    const unsigned int row = get_global_id(0);
    const unsigned int index = column * width + row;
